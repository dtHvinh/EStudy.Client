...
export default function FlashCardSetList({
  ...
  return (
    <div className="dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-8 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-3">
      {sets.map((set) => (
        <FlashCardSet
          {...set}
          key={set.id}
